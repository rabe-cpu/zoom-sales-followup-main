...
Drive保存は drive.file、黄色ハイライトは documents スコープで動かす。
既存refresh_tokenもこの最小権限で発行されているため、full drive scope は要求しない。

必要な環境変数:
    GOOGLE_OAUTH_CLIENT_ID / GOOGLE_OAUTH_CLIENT_SECRET / GOOGLE_OAUTH_REFRESH_TOKEN
    GOOGLE_DRIVE_FOLDER_ID … 保存先フォルダID（まなさんのマイドライブ内）
    HTTPLIB2_CA_CERTS / SSL_CERT_FILE / REQUESTS_CA_BUNDLE … TLSプロキシ環境のCA bundle（必要時）
"""
...
SCOPES = [
    # このRoutineが触るのは、OAuthアプリが作成/認識できる出力物と台帳だけ。
    # 既存refresh_tokenもこの権限で発行されているため、full driveを要求しない。
...
class GoogleDriveClient:
    def __init__(self):
        ...
        self.service = build("drive", "v3", http=_authorized_http(creds), cache_discovery=False)
        self.docs = build("docs", "v1", http=_authorized_http(creds), cache_discovery=False)
